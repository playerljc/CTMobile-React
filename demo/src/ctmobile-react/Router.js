...
export function Link(props) {
  ...
  const ctMobile = useContext(GlobalContext);
  ...
  return (
    <a
      ...
      style={style}
      onClick={() => {
        const { pageId = '', parameter = '', reload = ctMobile.config.linkCaptureReload } = props;
...
export function Back(props) {
  ...
  return (
    <a className={className} style={style} onClick={() => ctMobile.router.go(-1)}>
      {children}
    </a>
  );
}
